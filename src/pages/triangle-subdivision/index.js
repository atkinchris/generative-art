...
const sketch = p => {
  ...
  const distance = points => Math.sqrt((points[2] - points[0]) ** 2 + (points[3] - points[1]) ** 2)
  ...
  const subDivide = points => {
    ...
    const midPoint = p.createVector((points[0] + points[2]) / 2, (points[1] + points[3]) / 2)
    ...
    stack.push([points[4], points[5], points[0], points[1], midPoint.x, midPoint.y])

    stack.push([points[4], points[5], points[2], points[3], midPoint.x, midPoint.y])

    return true
  }

  const drawTriangle = points => {
    const hue = p.noise(points[0] * noiseScale, points[1] * noiseScale) * 127 + 127
